...
| ID | 전이 | 테스트 함수 | 상태 |
|---|---|---|---|
| TC-03/TC-04 | IN_STOCK → IN_STOCK (여유 재고 주문) | `test_successful_order_decrements_stock_and_returns_order`, `test_order_with_quantity_greater_than_one` | ✅ 기존 |
| TC-07 | IN_STOCK → SOLD_OUT (경계 전이) | `test_stock_reaches_zero_becomes_sold_out` | ✅ 기존 |
| TC-06 | SOLD_OUT → IN_STOCK (취소로 복구) | `test_after_cancel_another_customer_can_purchase_immediately` | ✅ 기존 |
| TC-18 | SOLD_OUT → IN_STOCK (운영자 수동 수정으로 복구) | `test_admin_restock_sold_out_product_makes_it_available_again` | 🆕 신규 |
| TC-08 | SOLD_OUT → SOLD_OUT (불허 전이) | `test_ordering_sold_out_product_returns_409` | ✅ 기존 |
| TC-05 | (없음) → COMPLETED → CANCELLED | `test_cancel_order_restores_stock` | ✅ 기존 |
| TC-14 | CANCELLED → CANCELLED (불허 전이) | `test_cancelling_already_cancelled_order_returns_409` | ✅ 기존 |
...
|---|---|---|---|---|---|
| TC-16 | -1 | 하한 미만 | 400 INVALID_QUANTITY | `test_order_with_negative_quantity_returns_400` | 🆕 신규 |
| TC-11 | 0 | 하한 경계값 | 400 INVALID_QUANTITY | `test_order_with_invalid_quantity_returns_400` | ✅ 기존 |
| TC-03 | 1 | 하한 바로 위 | 201 성공 | `test_successful_order_decrements_stock_and_returns_order` | ✅ 기존 |

### 2-2. quantity vs stock 상한 (동적 경계, stock=N 기준)

| ID | 값 | 위치 | 기대 결과 | 테스트 함수 | 상태 |
|---|---|---|---|---|---|
| TC-04 | N-1 | 상한 미만 | 201 성공, 재고 여유 | `test_order_with_quantity_greater_than_one` | ✅ 기존 |
| TC-07 | N (정확히 소진) | **상한 경계값** | 201 성공, 재고=0, SOLD_OUT 전환 | `test_stock_reaches_zero_becomes_sold_out` | ✅ 기존 (상태전이 TC-07과 동일 지점) |
| TC-17 | N+1 | 상한 바로 초과 | 409 SOLD_OUT | `test_order_with_quantity_exactly_one_more_than_stock_returns_409` | 🆕 신규 |
| TC-09 | N+100 | 상한 훨씬 초과(극단값) | 409 SOLD_OUT | `test_order_quantity_exceeds_stock_returns_409` | ✅ 기존 |
...
|---|---|---|---|---|
| TC-08 | 0 | 이미 품절 상태에서 주문 시도 | `test_ordering_sold_out_product_returns_409` | ✅ 기존 |
| TC-07, TC-23~25 | 1 | 마지막 재고 (동시성 시나리오의 전제) | `test_stock_reaches_zero_becomes_sold_out`, 5장 동시성 테스트 전체 | ✅ 기존 |
...
| ID | 클래스 | 테스트 함수 | 상태 |
|---|---|---|---|
| TC-01 | 유효 (존재하는 상품) | `test_get_stock_returns_current_quantity` 외 다수 | ✅ 기존 |
| TC-12, TC-13 | 무효 (존재하지 않는 상품) | `test_get_stock_for_unknown_product_returns_404`, `test_order_for_unknown_product_returns_404` | ✅ 기존 |

### 3-2. orderId(취소 대상) — 3개 클래스

| ID | 클래스 | 테스트 함수 | 상태 |
|---|---|---|---|
| TC-05 | 유효 (존재 + 아직 취소 안 됨) | `test_cancel_order_restores_stock` | ✅ 기존 |
| TC-15 | 무효① (존재하지 않는 주문) | `test_cancelling_unknown_order_returns_404` | ✅ 기존 |
| TC-14 | 무효② (이미 취소된 주문) | `test_cancelling_already_cancelled_order_returns_409` | ✅ 기존 |
...
|---|---|---|---|---|---|---|---|
| TC-19 | R1 | 운영자 수정 → 주문 | 예 | 성공(201) | 수정값 − 주문수량 | `test_order_after_admin_restock_succeeds` | 🆕 신규 |
| TC-20 | R2 | 운영자 수정 → 주문 | 아니오 | 품절(409) | 수정값 그대로 | `test_order_after_admin_sets_insufficient_stock_returns_409` | 🆕 신규 |
| TC-21 | R3 | 주문 → 운영자 수정 | 예 | 성공(201) | **운영자 지정값** (주문 결과와 무관하게 덮어써짐) | `test_admin_update_after_order_overwrites_final_stock` | 🆕 신규 |
| TC-22 | R4 | 주문 → 운영자 수정 | 아니오(품절) | 품절(409) | **운영자 지정값** | `test_admin_update_after_failed_order_sets_exact_value` | 🆕 신규 |
...
| ID | 테스트 함수 | 검증 내용 |
|---|---|---|
| TC-23 | `test_two_near_simultaneous_orders_on_last_unit_only_one_succeeds` | **과제가 명시한 시나리오 그 자체** — 실제 타이밍 재현(Barrier), 결과만 엄격 검증 |
| TC-24 | `test_lock_serializes_requests_even_under_artificial_delay` | 인위적 지연으로 락 경합을 강제해 결정적으로 재검증 |
| TC-25 | `test_no_oversell_under_high_concurrency` | 10건 동시 요청에도 오버셀 없음 (강도를 높인 추가 검증) |
