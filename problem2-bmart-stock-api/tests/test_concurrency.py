"""
동시성 테스트 (TC-23~TC-25) — '재고 1개 남은 상품에 두 요청이 수십 ms 간격으로 도달' 시나리오

설계기법 상세는 ../TEST_DESIGN.md 5장, ../TEST_CASES.md 참고.

설계 의도
    1. test_tc23_concurrent_orders_on_last_unit (TC-23, 과제가 명시한 시나리오 그 자체)
    ...
    2. test_tc24_lock_serializes_under_artificial_delay (TC-24)
    ...
    3. test_tc25_no_oversell_under_high_concurrency (TC-25)
...
def test_tc23_concurrent_orders_on_last_unit(last_unit_server):
    """
    TC-23 | 동시성 | 과제가 명시한 시나리오 그 자체
    Given: 재고 1개인 인기 상품이 있다
    When:  고객 A, B가 Barrier로 동시에 출발해 거의 동시에 주문하면 (수십 ms 간격)
    Then:  정확히 1건은 성공(201), 1건은 품절(409)이고 최종 재고는 0이다
    """
...
def test_tc24_lock_serializes_under_artificial_delay(last_unit_server):
    """
    TC-24 | 동시성 | 강제 경합으로 락 정합성을 결정적으로 증명
    Given: 재고 1개인 상품이 있다
    When:  lock 내부에 80ms 지연을 주입해 두 요청이 critical section 안에서 반드시 겹치게 만들면
           (Barrier 타이밍만으로는 우연히 안 겹칠 수 있어, 이 케이스가 '항상 재현되는' 회귀 방지망 역할)
    Then:  그래도 정확히 1건 성공 + 1건 품절이다 (재고가 음수가 되거나 둘 다 성공하지 않는다)
    """
    server = last_unit_server
    barrier = threading.Barrier(2)
...
def test_tc25_no_oversell_under_high_concurrency(last_unit_server):
    """
    TC-25 | 동시성 | 강도를 높인 추가 검증
    Given: 재고 1개인 상품이 있다
    When:  10개의 요청을 동시에 던지면
    Then:  정확히 1건만 성공(201)하고 나머지 9건은 품절(409)이다 (oversell 방지)
    """
    server = last_unit_server
    n_requests = 10
