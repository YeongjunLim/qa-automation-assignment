...
```bash
# 문제2: B마트 재고 API 자동화 테스트 (18개 케이스, 전부 통과 확인됨)
...
| 2. B마트 재고 API | 코드 | Python (Flask + pytest) | 자체 구현 Mock 서버(락 기반 동시성 제어) + 18개 테스트(동시성 3개 포함), 전부 통과 |
...
- **실행 검증**: 문제2 pytest 스위트(18개 테스트 전부 통과)와 문제4 프로토타입 스크립트를 실제로 로컬에서 실행해 동작을 확인했습니다. 이 과정에서 프로토타입의 프롬프트 템플릿 치환 로직에 있던 버그(JSON 스키마 예시의 중괄호가 `str.format()`과 충돌)를 실행 테스트로 직접 발견하고 수정했습니다.
